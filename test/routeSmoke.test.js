...
test('/sponsors and /media-kit stay wired to public sponsor pages', () => {
  ...
  assert.match(sponsorScreenSource, /Sponsor readiness/);
  assert.match(sponsorScreenSource, /How sponsorship works/);
  assert.match(sponsorScreenSource, /Placement map/);
  assert.match(sponsorScreenSource, /Brand safety standard/);
  assert.match(sponsorScreenSource, /After you send an inquiry/);
...
test('/next stays wired to the public next-event lobby', () => {
  ...
  assert.match(nextScreenSource, /getTournamentDiscoveryPresentation/);
  assert.match(nextScreenSource, /stickyActions=\{false\}/);
  assert.match(nextScreenSource, /REGISTERED/);
  assert.doesNotMatch(nextScreenSource, /NextLobbyHero/);
...
test('phase 1 signup capture and public counts stay wired through Netlify Functions and Blobs', () => {
  ...
  assert.match(tournamentBracketSource, /payload\.allowEarly !== true/);
  ...
  assert.match(hubUiSource, /Return to Match/);
  assert.match(nextScreenSource, /TournamentJourney/);
  assert.match(nextScreenSource, /MasterTournamentCard/);
  assert.match(nextScreenSource, /TournamentDiscoveryList/);
  ...
  assert.match(nextScreenSource, /TOURNAMENT DISCOVERY/);
  assert.match(nextScreenSource, /REGISTER/);
  assert.match(nextScreenSource, /getTournamentDiscoveryPresentation/);
  ...
  assert.match(tournamentScreenSource, /PlayerStatusSpotlight/);
  assert.match(tournamentScreenSource, /getTournamentPlayerPresentation/);
  assert.match(tournamentScreenSource, /PLAY MATCH/);
  assert.match(tournamentScreenSource, /variant="success"/);
  assert.match(tournamentScreenSource, /checkIn\?\.window \|\| visibleTournament\.checkIn\?\.preview/);
  assert.match(tournamentScreenSource, /const steps = statusUnavailable \? \[\] : \[/);
  ...
  assert.match(checkInScreenSource, /YOU’RE REGISTERED/);
  assert.match(checkInScreenSource, /SIGNING UP…/);
  ...
  assert.match(checkInScreenSource, /Sign Up is the next separate step/);
  assert.doesNotMatch(checkInScreenSource, /Join Tournament/);
