...
export const APPLE_RELEASE_LABELS = Object.freeze({
  ...
  [APPLE_RELEASE_STATES.UPDATE_PENDING]: 'Current version available — update pending',
});

export const PLATFORM_SUBMISSION_STATEMENT = 'Spades and Euchre submitted to Apple.';
...
export const PLATFORM_GAME_PRESENTATION = Object.freeze({
  spades: Object.freeze({
    ...
    description: 'Head-to-head Spades with ranked play, bot practice, and tournament assignments.',
    capabilities: Object.freeze(['Ranked', 'Bot practice', 'Public tournaments']),
  }),
  euchre: Object.freeze({
    slug: 'euchre',
    name: '1v1 Euchre',
    releaseState: APPLE_RELEASE_STATES.SUBMITTED,
    description: 'Server-authoritative head-to-head Euchre with ranked play and invited competition.',
    capabilities: Object.freeze(['Ranked', 'Bot practice', 'Invited tournaments']),
  }),
});
...
export function findRedditSundayCommunityCup(tournaments = []) {
  return tournaments.find((tournament) => tournament?.title === REDDIT_SUNDAY_SPADES_TITLE)
    || tournaments.find((tournament) => (
      String(tournament?.gameSlug || tournament?.game || 'spades').toLowerCase() === 'spades'
...
export function getTournamentBroadcastPath(slug) {
  return slug ? `/overlay/bracket?slug=${encodeURIComponent(slug)}` : '/overlay/bracket';
}
...
export function getCommunityCupPrimaryAction({
  ...
  if (status === 'completed' || status === 'complete' || data?.nextStep === 'complete' || data?.nextStep === 'champion') {
  ...
  if (!data?.account) {
    if (hasBracket) {
      return { label: 'Sign in for match', href: `${signupPath}?mode=signin#account-access` };
    }

    if (registrationStatus === 'open') {
      return { label: 'Sign in to register', href: `${signupPath}?mode=signin#account-access` };
    }
  ...
  if (data.signup) {
    ...
    if (registrationStatus === 'check-in' && !signupStatus.includes('checked')) {
...
export function getFeaturedCompetitionAction({
  ...
  if (status === 'cancelled') {
    return { label: 'View cancellation', href: tournamentPath };
  }

  if (hasBracket || status === 'live') {
    return { label: 'My Match', href: matchPath };
  }
  ...
  if (registrationStatus === 'full' || registrationStatus === 'waitlist') {
    return { label: 'Join waitlist', href: signupPath };
  }
