...
The receiver validates the allowlisted schema again, ignores client timestamps, recomputes Core Web Vital ratings, and stores one anonymous aggregate per UTC day. Concurrent updates use ETag checks. It never stores raw events, so the report can show page views and clicks but cannot claim unique visitors.
